...
def model_evaluate(Y, P, dataset, epoch, writer, iter_num):
    thresholds = {"davis": 7.0, "kiba": 12.1} # from AttentionDTA
    mse, cindex, rm2, pearson, aupr = get_mse(Y, P), get_cindex(Y, P), get_rm2(Y, P), get_pearson(Y, P), get_aupr(Y, P, thresholds[dataset])
    
    if writer is not None:
        writer.add_scalars('infer/mse', {f'{iter_num}':mse}, epoch)
        writer.add_scalars('infer/ci', {f'{iter_num}':cindex}, epoch)
        writer.add_scalars('infer/rm2', {f'{iter_num}':rm2}, epoch)
        writer.add_scalars('infer/pearson', {f'{iter_num}':pearson}, epoch)
        writer.add_scalars('infer/aupr', {f'{iter_num}':aupr}, epoch)

    return (mse, cindex, rm2, pearson, aupr)
...
if __name__ == '__main__':
    ...
    hit_num = 20
    all_num = 99
    for i in range(0, hit_num+1, 1):
        cur_num = comb(hit_num, i) * comb(all_num-hit_num, hit_num-i)
        rate += ((1 / hit_num) * i * cur_num)
        num += cur_num
    print(rate / num)
